...
import azureml._vendor.ruamel.yaml
from azureml._vendor.ruamel.yaml.error import UnsafeLoaderWarning, YAMLError  # NOQA

from azureml._vendor.ruamel.yaml.tokens import *  # NOQA
from azureml._vendor.ruamel.yaml.events import *  # NOQA
from azureml._vendor.ruamel.yaml.nodes import *  # NOQA

from azureml._vendor.ruamel.yaml.loader import BaseLoader, SafeLoader, Loader, RoundTripLoader  # NOQA
from azureml._vendor.ruamel.yaml.dumper import BaseDumper, SafeDumper, Dumper, RoundTripDumper  # NOQA
from azureml._vendor.ruamel.yaml.compat import StringIO, BytesIO, with_metaclass, nprint, nprintf  # NOQA
from azureml._vendor.ruamel.yaml.resolver import VersionedResolver, Resolver  # NOQA
from azureml._vendor.ruamel.yaml.representer import (
    BaseRepresenter,
    SafeRepresenter,
    Representer,
    RoundTripRepresenter,
)
from azureml._vendor.ruamel.yaml.constructor import (
    BaseConstructor,
    SafeConstructor,
    Constructor,
    RoundTripConstructor,
)
from azureml._vendor.ruamel.yaml.loader import Loader as UnsafeLoader
from azureml._vendor.ruamel.yaml.comments import CommentedMap, CommentedSeq, C_PRE

if False:  # MYPY
    from typing import List, Set, Dict, Union, Any, Callable, Optional, Text  # NOQA
    from azureml._vendor.ruamel.yaml.compat import StreamType, StreamTextType, VersionType  # NOQA
...
class YAML:
    def __init__(self, *, typ=None, pure=False, output=None, plug_ins=None):  # input=None,
        ...
        self.Resolver = azureml._vendor.ruamel.yaml.resolver.VersionedResolver  # type: Any
        ...
        if 'rt' in self.typ:
            ...
            self.Emitter = (
                azureml._vendor.ruamel.yaml.emitter.Emitter if pure or CEmitter is None else CEmitter
            )
            self.Representer = azureml._vendor.ruamel.yaml.representer.SafeRepresenter
            self.Parser = azureml._vendor.ruamel.yaml.parser.Parser if pure or CParser is None else CParser
            self.Composer = azureml._vendor.ruamel.yaml.composer.Composer
            self.Constructor = azureml._vendor.ruamel.yaml.constructor.SafeConstructor
        elif 'base' in self.typ:
            self.Emitter = azureml._vendor.ruamel.yaml.emitter.Emitter
            self.Representer = azureml._vendor.ruamel.yaml.representer.BaseRepresenter
            self.Parser = azureml._vendor.ruamel.yaml.parser.Parser if pure or CParser is None else CParser
            self.Composer = azureml._vendor.ruamel.yaml.composer.Composer
            self.Constructor = azureml._vendor.ruamel.yaml.constructor.BaseConstructor
        elif 'unsafe' in self.typ:
            self.Emitter = (
                azureml._vendor.ruamel.yaml.emitter.Emitter if pure or CEmitter is None else CEmitter
            )
            self.Representer = azureml._vendor.ruamel.yaml.representer.Representer
            self.Parser = azureml._vendor.ruamel.yaml.parser.Parser if pure or CParser is None else CParser
            self.Composer = azureml._vendor.ruamel.yaml.composer.Composer
            self.Constructor = azureml._vendor.ruamel.yaml.constructor.Constructor
        elif 'rtsc' in self.typ:
            self.default_flow_style = False
            # no optimized rt-dumper yet
            self.Emitter = azureml._vendor.ruamel.yaml.emitter.Emitter
            self.Serializer = azureml._vendor.ruamel.yaml.serializer.Serializer
            self.Representer = azureml._vendor.ruamel.yaml.representer.RoundTripRepresenter
            self.Scanner = azureml._vendor.ruamel.yaml.scanner.RoundTripScannerSC
            # no optimized rt-parser yet
            self.Parser = azureml._vendor.ruamel.yaml.parser.RoundTripParserSC
            self.Composer = azureml._vendor.ruamel.yaml.composer.Composer
            self.Constructor = azureml._vendor.ruamel.yaml.constructor.RoundTripConstructor
        ...
        if setup_rt:
            ...
            self.Serializer = azureml._vendor.ruamel.yaml.serializer.Serializer
            self.Representer = azureml._vendor.ruamel.yaml.representer.RoundTripRepresenter
            self.Scanner = azureml._vendor.ruamel.yaml.scanner.RoundTripScanner
            # no optimized rt-parser yet
            self.Parser = azureml._vendor.ruamel.yaml.parser.RoundTripParser
            self.Composer = azureml._vendor.ruamel.yaml.composer.Composer
            self.Constructor = azureml._vendor.ruamel.yaml.constructor.RoundTripConstructor
        del setup_rt
        self.stream = None
    ...
    def get_constructor_parser(self, stream):
        ...
        if self.Parser is not CParser:
            if self.Reader is None:
                self.Reader = azureml._vendor.ruamel.yaml.reader.Reader
            if self.Scanner is None:
                self.Scanner = azureml._vendor.ruamel.yaml.scanner.Scanner
            self.reader.stream = stream
        else:
            if self.Reader is not None:
                if self.Scanner is None:
                    self.Scanner = azureml._vendor.ruamel.yaml.scanner.Scanner
                self.Parser = azureml._vendor.ruamel.yaml.parser.Parser
                self.reader.stream = stream
    ...
    def get_serializer_representer_emitter(self, stream, tlca):
        ...
        rslvr = (
            azureml._vendor.ruamel.yaml.resolver.BaseResolver
            if 'base' in self.typ
            else azureml._vendor.ruamel.yaml.resolver.Resolver
        )
...
def warn_deprecation(fun, method, arg=''):
    ...
    from azureml._vendor.ruamel.yaml.compat import _F
...
def add_implicit_resolver(
    ...
    if Loader:
        if hasattr(Loader, 'add_implicit_resolver'):
            ...
            Loader, (BaseLoader, SafeLoader, azureml._vendor.ruamel.yaml.loader.Loader, RoundTripLoader)
        ):
            Resolver.add_implicit_resolver(tag, regexp, first)
        else:
            raise NotImplementedError
    if Dumper:
        if hasattr(Dumper, 'add_implicit_resolver'):
            ...
            Dumper, (BaseDumper, SafeDumper, azureml._vendor.ruamel.yaml.dumper.Dumper, RoundTripDumper)
        ):
            Resolver.add_implicit_resolver(tag, regexp, first)
        else:
            raise NotImplementedError
...
def add_multi_constructor(tag_prefix, multi_constructor, Loader=None, constructor=Constructor):
    ...
    if Loader is None:
        ...
        elif issubclass(Loader, azureml._vendor.ruamel.yaml.loader.Loader):
            Constructor.add_multi_constructor(tag_prefix, multi_constructor)
        elif issubclass(Loader, RoundTripLoader):
