...
class AzCommandsLoader(CLICommandsLoader):  # pylint: disable=too-many-instance-attributes
    ...
    def _apply_doc_string(self, dest, command_kwargs):
        ...
        if not isinstance(doc_string_source, str):
            raise AzureMLException("command authoring error: applying doc_string_source '{}' directly will cause "
                                   "slowdown. Import by string name instead.".format(doc_string_source.__name__))
        ...
        if not model:
            raise AzureMLException("command authoring error: source '{}' not found.".format(doc_string_source))
