...
def get_folded_parameter_help_string(
    ...
    if default_none and not allow_none:
        raise AzureMLException('Cannot use default_none=True and allow_none=False')
...
def get_folded_parameter_validator(
    ...
    if has_parent and not all(parent_params):
        raise AzureMLException('All parent parameters must be specified (name, type, option) if any are.')

    if default_none and not allow_none:
        raise AzureMLException('Cannot use default_none=True if allow_none=False')
    ...
    def validator(cmd, namespace):
        ...
        if property_val in ('', '""', "''") or (property_val is None and default_none):
            if not allow_none:
                raise AzureMLException('{} cannot be None.'.format(property_option))
        ...
        if value_was_id:
            ...
            raise AzureMLException('{} {} does not exist. {}'.format(
