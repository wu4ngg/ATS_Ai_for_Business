    ...
    def load(self):
        with CrossPlatLock(self._lock_file):
            try:
                ...
                raise AzureMLException("Failed to load token files. If you can reproduce, "
                                       "please log an issue at https://github.com/Azure/azure-cli/issues. At the same "
                                       "time, you can clean up by running 'az account clear' and "
                                       "then 'az login'. (Inner Error: {})".format(ex))
