...
from azureml._vendor.azure_cli_core.util import reload_module, rmtree_with_retry
...
def _whl_download_from_url(url_parse_result, ext_file):
    ...
    if r.status_code != 200:
        raise AzureMLException("Request to {} failed with {}".format(url, r.status_code))
...
def _add_whl_ext(cli_ctx, source, ext_sha256=None, pip_extra_index_urls=None, pip_proxy=None, system=None):  # pylint: disable=too-many-statements
    ...
    if not extension_name:
        raise AzureMLException('Unable to determine extension name from {}. Is the file name correct?'.format(source))
    if extension_exists(extension_name, ext_type=WheelExtension):
        raise AzureMLException('The extension {} already exists.'.format(extension_name))
    ...
    if is_url:
        ...
        try:
            ...
            raise AzureMLException('Please ensure you have network connection. Error detail: {}'.format(str(err)))
        logger.debug('Downloaded to %s', ext_file)
    else:
        # Get file path
        ext_file = os.path.realpath(os.path.expanduser(source))
        if not os.path.isfile(ext_file):
            raise AzureMLException("File {} not found.".format(source))
    ...
    if ext_sha256:
        ...
        if valid_checksum:
            ...
            raise AzureMLException("The checksum of the extension does not match the expected value. "
    ...
    try:
        ...
        raise AzureMLException('The extension is invalid. Use --debug for more information.')
    ...
    if pip_status_code > 0:
        ...
        raise AzureMLException('An error occurred. Pip failed with status code {}. '
...
def _install_deps_for_psycopg2():  # pylint: disable=too-many-statements
    ...
    if system == 'Darwin':
        ...
        if installer == 'DEB' or any(x in distname for x in ['ubuntu', 'debian']):
            ...
            if in_cloud_console():
                raise AzureMLException("This extension is not supported in Cloud Shell as you do not have permission to install extra dependencies.")
...
def update_extension(cmd=None, extension_name=None, index_url=None, pip_extra_index_urls=None, pip_proxy=None, cli_ctx=None, version=None):
    try:
        ...
        try:
            ...
            raise AzureMLException('Failed to update. Rolled {} back to {}.'.format(extension_name, cur_version))
        CommandIndex().invalidate()
    except ExtensionNotInstalledException as e:
        raise AzureMLException(str(e))
...
def list_versions(extension_name, index_url=None, cli_ctx=None):
    ...
    try:
        ...
        raise AzureMLException('Extension {} not found.'.format(extension_name))
