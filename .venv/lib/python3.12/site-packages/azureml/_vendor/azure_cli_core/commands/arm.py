...
def handle_long_running_operation_exception(ex):
    ...
    cli_error = AzureMLException(error_message)
    ...
    def parse_ids_arguments(_, command, args):
        ...
        if not ids:
            ...
            if errors:
                ...
                raise AzureMLException('({} | {}) are required'.format(missing_required, '--ids'))
            return
        ...
        for val in full_id_list:
            if not is_valid_resource_id(val):
                raise AzureMLException('invalid resource ID: {}'.format(val))
            ...
            for arg in combined_args:
                ...
                if id_value is None:
                    ...
                    raise AzureMLException("Argument {arg_name} cannot be derived from ID {id}. "
                                           "Please provide a complete resource ID "
                                           "containing all information of '{group_name}' "
                                           "arguments. ".format(
                                                                id=val, arg_name=argument_name,
...
def _get_operations_tmpl(cmd, custom_command=False):
    ...
    if not operations_tmpl:
        raise AzureMLException("command authoring error: cmd '{}' does not have an operations_tmpl.".format(cmd.name))
...
def set_properties(instance, expression, force_string):
    ...
    if key is None or key.strip() == '':
        raise AzureMLException('usage error: Empty key in --set. Correct syntax: --set KEY=VALUE [KEY=VALUE ...]')
    ...
    try:
        ...
        raise AzureMLException('index {} doesn\'t exist on {}'.format(index_value, name))
...
def remove_properties(instance, argument_values):
    ...
    if not list_index:
        ...
        try:
            ...
            raise AzureMLException('index {} doesn\'t exist on {}'.format(list_index, list_attribute_path[-1]))
        except AttributeError:
            raise AzureMLException('{} doesn\'t exist'.format(list_attribute_path[-1]))
    ...
    try:
        ...
        if index and '=' in index.group(1):
            ...
            if len(matches) == 1:
                return matches[0]
            if len(matches) > 1:
                raise AzureMLException("non-unique key '{}' found multiple matches on {}. Key must be unique.".format(
                    key, path[-2])
                )
            if key in getattr(instance, 'additional_properties', {}):
                instance.enable_additional_properties_sending()
                return instance.additional_properties[key]
            raise AzureMLException("item with value '{}' doesn\'t exist for key '{}' on {}".format(
                value, key, path[-2])
            )
        ...
        if index:
            try:
                ...
                raise AzureMLException('index {} doesn\'t exist on {}'.format(index_value, path[-2]))
...
def resolve_role_id(cli_ctx, role, scope):
    ...
    if re.match(r'/subscriptions/[^/]+/providers/Microsoft.Authorization/roleDefinitions/',
        ...
        if not role_id:  # retrieve role id
            ...
            if not role_defs:
                raise AzureMLException("Role '{}' doesn't exist.".format(role))
            if len(role_defs) > 1:
                ids = [r.id for r in role_defs]
                err = "More than one role matches the given name '{}'. Please pick an id from '{}'"
                raise AzureMLException(err.format(role, ids))
...
def get_arm_resource_by_id(cli_ctx, arm_id, api_version=None):
    ...
    if not is_valid_resource_id(arm_id):
        raise AzureMLException("'{}' is not a valid ID.".format(arm_id))
    ...
    if not api_version:
        ...
        try:
            ...
            raise AzureMLException(err.format(resource_type_str))
