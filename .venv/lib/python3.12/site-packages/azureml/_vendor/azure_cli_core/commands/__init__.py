...
class CacheObject:
    ...
    def path(self, args, kwargs):
        ...
        if not subscription_id:
            raise AzureMLException('subscription ID unexpectedly empty')
        if not cli_ctx.cloud.name:
            raise AzureMLException('cloud name unexpectedly empty')
        copy_kwargs = kwargs.copy()
        copy_kwargs.pop('self', None)
        resource_group = copy_kwargs.pop('resource_group_name', None) or args[0]

        if len(args) > 2:
            raise AzureMLException('expected 2 args, got {}: {}'.format(len(args), args))
        if len(copy_kwargs) > 1:
            raise AzureMLException('expected 1 kwarg, got {}: {}'.format(len(copy_kwargs), copy_kwargs))
...
def get_property(items, name):
    ...
    if not result:
        raise AzureMLException("Property '{}' does not exist".format(name))
    return result
...
class AzCliCommandInvoker(CommandInvoker):
    ...
    def execute(self, args):
        ...
        if exceptions:
            ...
            if not results:
                return CommandResultItem(None, exit_code=1, error=AzureMLException('Encountered more than '
                                                                                   'one exception.'))
...
class AzCommandGroup(CommandGroup):
    ...
    def _apply_tags(self, merged_kwargs, kwargs, command_name):
        ...
        if is_preview and is_experimental:
            raise AzureMLException(PREVIEW_EXPERIMENTAL_CONFLICT_ERROR.format("command",
